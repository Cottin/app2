...
export createFlow = ({data: initialData, selectors: selectorsDef, onStateChange}) ->
	data = initialData
	...
	resetData = (spec) ->
		t0 = performance.now()
		data = change spec, initialData
		[derived, ran] = runSelectors data, {}, selectors
		state = {...data, ...derived}
		onStateChange? state
		log 'RESET', performance.now() - t0, ran, true, spec
		listeners.forEach (l) -> l()
	...
	base = {get: getState, set: setData, use: useState, setManually: setDataManually, reset: resetData}
